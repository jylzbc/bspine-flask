...
class BridgeLayer_4(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if (type(inputs) == list):
            ...
            c1, c2, c3, c4 = inputs
            B, C, _, _= c1.shape
            c1f = c1.permute(0, 2, 3, 1).reshape(B, -1, C)  # 8192*64
            c2f = c2.permute(0, 2, 3, 1).reshape(B, -1, C)  # 4096*64
            c3f = c3.permute(0, 2, 3, 1).reshape(B, -1, C)  # 2560*64
            c4f = c4.permute(0, 2, 3, 1).reshape(B, -1, C)  # 1024*64
        ...
        tem1 = tx[:,:8192,:].reshape(B, -1, C) 
        tem2 = tx[:,8192:8192+4096,:].reshape(B, -1, C*2)
        tem3 = tx[:,8192+4096:8192+4096+2560,:].reshape(B, -1, C*5)
        tem4 = tx[:,8192+4096+2560:8192+4096+2560+1024,:].reshape(B, -1, C*8)

        m1f = self.mixffn1(tem1, 128, 64).reshape(B, -1, C)
        m2f = self.mixffn2(tem2, 64, 32).reshape(B, -1, C)
        m3f = self.mixffn3(tem3, 32, 16).reshape(B, -1, C)
        m4f = self.mixffn4(tem4, 16, 8).reshape(B, -1, C)
...
class BridegeBlock_4(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        sk1 = bridge4[:,:8192,:].reshape(B, 128, 64, C).permute(0,3,1,2) 
        sk2 = bridge4[:,8192:8192+4096,:].reshape(B, 64, 32, C*2).permute(0,3,1,2) 
        sk3 = bridge4[:,8192+4096:8192+4096+2560,:].reshape(B, 32, 16, C*5).permute(0,3,1,2) 
        sk4 = bridge4[:,8192+4096+2560:8192+4096+2560+1024,:].reshape(B, 16, 8, C*8).permute(0,3,1,2) 
...
class MISSFormer(nn.Module):
    def __init__(self, num_classes=9, token_mlp_mode="mix_skip", encoder_pretrained=True):
        ...
        self.decoder_3= MyDecoderLayer((d_base_feat_size,d_base_feat_size//2), in_out_chan[3], heads[3], reduction_ratios[3],token_mlp_mode, n_class=num_classes)
        self.decoder_2= MyDecoderLayer((d_base_feat_size*2,d_base_feat_size),in_out_chan[2], heads[2], reduction_ratios[2], token_mlp_mode, n_class=num_classes)
        self.decoder_1= MyDecoderLayer((d_base_feat_size*4,d_base_feat_size*2), in_out_chan[1], heads[1], reduction_ratios[1], token_mlp_mode, n_class=num_classes)
        self.decoder_0= MyDecoderLayer((d_base_feat_size*8,d_base_feat_size*4), in_out_chan[0], heads[0], reduction_ratios[0], token_mlp_mode, n_class=num_classes, is_last=True)
