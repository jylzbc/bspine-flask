...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False):
        ...
        self.fc_centerline=nn.Linear(512 * block.expansion, 34*2)
    ...
    def forward(self, x):
        ...
        centerline = self.fc_centerline(x)
        bspline = self.fc_bspline(centerline)
        angle = self.fc_angle(centerline)
        return torch.sigmoid(centerline), torch.sigmoid(bspline), torch.sigmoid(angle)
