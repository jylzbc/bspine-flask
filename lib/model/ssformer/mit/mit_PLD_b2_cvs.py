...
import torch
# from libs.utils.init_weights import init_weights
...
class Decoder2(nn.Module):
    def __init__(self,  n_classes, bilinear=False):
        ...
        self.up1 = (Up(832, 256 // factor, True))
        self.up2 = (Up(256, 128 // factor, bilinear))
        self.up3 = (Up(128, 64 // factor, bilinear))
        self.upct1=nn.ConvTranspose2d(64, 64 // 2, kernel_size=2, stride=2)
        self.upct2=nn.ConvTranspose2d(32, 32 // 2, kernel_size=2, stride=2)
        self.outc = (OutConv(16, n_classes))

        self.upf=UpsamplingBilinear2d(scale_factor=2)
    def forward(self, inputs):
        x2,x3,x4,x5=inputs
        x = self.up1(x5, x4)   
        x = self.up2(x, x3)        
...
class mit_PLD_b2(nn.Module):
    def __init__(self, class_num=2, pretrained=False, **kwargs):
        ...
        self.layer1 = self._make_layer(block=ResnetBlock, block_num=4,
                                       in_channels=4, out_channels=8, stride=1)
        self.layer2 = self._make_layer(block=ResnetBlock, block_num=4,
                                       in_channels=8, out_channels=4, stride=1)
        if pretrained:
            ...
            for m in self.modules():
                if isinstance(m, nn.Conv2d):
                    ...
                    init_weights(m, init_type='kaiming')
        # self.layer1 = self._make_layer(block=ResnetBlock, block_num=2,
        #                                in_channels=4, out_channels=8, stride=1)
        # self.layer2 = self._make_layer(block=ResnetBlock, block_num=2,
        #                                in_channels=8, out_channels=4, stride=1)

        self.decoder2=Decoder2(1)
    ...
    def forward(self, x):
        ...
        features_up1 = self.decode_head(features1)
        
        up1 = UpsamplingBilinear2d(scale_factor=4)
        features_up1 = up1(features_up1)
        features_up1 = torch.sigmoid(features_up1)
        out_spine = features_up1[:, :1]
        
        up2 = UpsamplingBilinear2d(scale_factor=4)
        feature_up2 = self.decoder2(features1)
        feature_up2 = up2(feature_up2)
        feature_up2 = torch.sigmoid(feature_up2)
        # out_ver = out_ver[:, 1:]
        out_ver = torch.nn.functional.interpolate(feature_up2, size=out_spine.shape[2:], mode='bilinear', align_corners=False)
        
        return out_spine, out_ver
