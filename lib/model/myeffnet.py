...
class EfficientNet(nn.Module):
    ...
    def __init__(self, width_mult=1.0, depth_mult=1.0, dropout_rate=0.2, in_channels=3, num_classes=1000):
        ...
        self.classifier = nn.Sequential(
            ...
            nn.Linear(last_channels, 34*2),
        )
        self.fc_bspline = nn.Sequential(
            nn.Linear(34*2, 128),nn.ReLU(inplace=True),
    ...
    def forward(self, x):
        ...
        centerline = self.classifier(x)
        bspline = self.fc_bspline(centerline)
        angles = self.fc_angle(centerline)
        return torch.sigmoid(centerline), torch.sigmoid(bspline), torch.sigmoid(angles)
